...
if __name__ == "__main__":
    mgr = RTDEManager("192.168.0.200", use_receive=True, use_control=True, use_io=True)
    ...
    if mgr.check_and_reconnect():
        ...
        try:
            ...
            # Save TCP to csv file
            import csv
            with open('tcp_pose.csv', 'a', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(['X', 'Y', 'Z', 'RX', 'RY', 'RZ'])
                writer.writerow(tcp_pose)
    ...
    #rtde_c.moveL([0/1000,-400/1000,600/1000,0,1.6,0])
